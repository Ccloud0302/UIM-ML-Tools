...
bool ESP8266::setIPAP(String addr)
{
    return sATCIPAP(addr);
}

bool ESP8266::setIPSTA(String addr)
{
    return sATCIPSTA(addr);
}
...
bool ESP8266::send(String buffer)
{
    return sATCIPSENDSingle(buffer.c_str(), buffer.length());
}
...
bool ESP8266::send(uint8_t mux_id, String buffer)
{
    return sATCIPSENDMultiple(mux_id, buffer.c_str(), buffer.length());
}
...
bool ESP8266::recvFindAndFilter(String target, String begin, String end, String &data, uint32_t timeout)
{
    ...
    if (data_tmp.indexOf(target) != -1) {
        ...
        int32_t index2 = data_tmp.indexOf(end,index1);
...
bool ESP8266::sATCIPAP(String addr)
{
    rx_empty();
...
bool ESP8266::sATCIPSTA(String addr)
{
    rx_empty();
