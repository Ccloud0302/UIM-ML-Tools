package com.markerhub.dal.impl;
...
public class KGraphRepository implements IKGraphRepository {
	...
	public List<HashMap<String, Object>> getdomainnodes(String domain) {
		...
		try {
			String domainSql = String.format("MATCH (n:`%s`) RETURN n", domain);
			if (!StringUtil.isBlank(domain)) {
	...
	public void updatedomainname(String oldName, String domainName) {
		try {
			String cypherSql = String.format(
					"match (n:`%s`) remove n:`%s` set n:`%s`", oldName, oldName, domainName);
			neo4jUtil.excuteCypherSql(cypherSql);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public HashMap<String, Object> getNodeAttrById(String domain, int nodeid) {
		...
		List<HashMap<String, Object>> graphNodeList = new ArrayList<HashMap<String, Object>>();
		try {
			String cypherSql = String.format("MATCH (n:`%s`) where id(n)=%s return n", domain, nodeid);
			graphNodeList = neo4jUtil.GetGraphNode(cypherSql);
			if (graphNodeList.size() > 0) {
				return graphNodeList.get(0);
			}
